...
MongooseReference.prototype.updateAdd = function(resource, referenceImage,referenceNews)  {
    ...
    var that = this;
    //TODO remove from old
    console.log("reference image: " + JSON.stringify(referenceImage));
    console.log("resource news: " + JSON.stringify(resource.documentationJson.model[path.name]));
    this.path = resource.documentationJson.model[path.name];

    var type =  resource.documentationJson.type;
    console.log("type: " +type);
    var ReferencingDoc = mongoose.model(type);

    console.log("reffield news: " + referenceNews);
    console.log("reffield image: " + referenceImage);
    ReferencingDoc.findById( referenceNews._id , function(err, obj) {
        if(obj[path.name] instanceof Array) {

            obj[path.name].remove(referenceImage);
        }

        obj.save(that.errorCallback);
    });
