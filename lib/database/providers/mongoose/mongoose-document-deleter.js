        ...
        if(err) {
            ...
            for (var property in resource.documentationJson.model) {
                ...
                if (path.hasOwnProperty(Reference.ReferenceRuleProperty)) {
                    ...
                    var ref = new Reference(path, {}, obj, onRemoveFinished);
                    ref.remove(onRemoveFinished);

                }
            }
            if(!hasReferenceToRemove) {
                var callbackObject = {
                    success : true,
                    error : undefined,
                    obj:undefined,
                    path:"hasNoReferenceToRemove"
                }
                onRemoveFinished(callbackObject)
            }
